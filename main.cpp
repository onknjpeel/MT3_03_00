...
Vector3 Lerp(const Vector3& v1, const Vector3& v2, float t) {
	Vector3 a;
	a = { v1.x * t,v1.y * t,v1.z * t };
	Vector3 b;
	b = { (1.0f - t) * v2.x,(1.0f - t) * v2.y,(1.0f - t) * v2.z };
	return Add(a, b);
}
...
void DrawBezier(const Vector3& controlPoint0, const Vector3& controlPoint1, const Vector3& controlPoint2,
	...
	float t0;
	float t1;
	Vector3 bezier0[subdivision];
	Vector3 bezier1[subdivision];
	for (int i = 0; i < subdivision; i++) {
		t0 = float(i) / float(subdivision);
		t1 = float(i + 1) / float(subdivision);

		bezier0[i] = Bezier(controlPoint0, controlPoint1, controlPoint2, t0);
		bezier1[i] = Bezier(controlPoint0, controlPoint1, controlPoint2, t1);

		bezier0[i] = Transform(Transform(bezier0[i], viewProjectionMatrix), viewportMatrix);
		bezier1[i] = Transform(Transform(bezier1[i], viewProjectionMatrix), viewportMatrix);

		Novice::DrawLine(int(bezier0[i].x), int(bezier0[i].y),int(bezier1[i].x), int(bezier1[i].y), color);
